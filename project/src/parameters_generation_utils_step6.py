...
def alpha_generation(alpha_parameters, seed, nr_classes=3):
    alphas = np.zeros((nr_classes,6))
    for user_class in range(0,nr_classes):
...
def prob_matrix_generation(primary_to_secondary_mapping, lambda_parameter, nr_classes=3):
    prob_matrix = np.zeros((nr_classes,5,5))
    for user_class in range(0,nr_classes):
