...
class Estimator_Thread(Thread):

    def __init__(self, items, dataset, node_index):
    ...
    def estimate_node_probabilities(self):
        ...
        for episode in self.dataset:
            idx_w_active = np.argwhere(episode[:, self.node_index] == 1).reshape(-1)
        ...
        self.value = estimated_prob
    
    def run(self):
        self.estimate_node_probabilities()
...
class Probabilities_Estimator:
    ...
    def estimate_probabilities(self, n_episodes=1000, n_steps_max=10):
        ...
        for node_index in range(n_nodes):
            thread = Estimator_Thread(self.items, dataset, node_index)
