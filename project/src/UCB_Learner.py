...
class UCB_Learner(Learner):
    def __init__(self, prices):
        ...
        self.previous_arms = np.zeros(prices.shape[1])
    ...
    def pull_prices(self):  # to pick the 5 arms, 1 for each item, return the indexes inside prices matrix
        ...
        for i in range(self.n_items):
            idx[i] = np.argmax((self.means[i, :] + self.widths[i, :]))
        return idx

    def reset(self):
        self.__init__(self.prices)

    def update(self, pulled_arms,
               rewards):  # pulled arms è un vettore di dim 5, rewards ha dim 5x(volte che pesco i rewards in una giornata)
